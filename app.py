# -*- coding: utf-8 -*-
"""
Post-based Watermark & Resize GUI — Phase 2 (controller 분리)
- UI: app.py (Tk)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Post Watermark & Resize (Phase 2)")
    ...
    def _build_ui(self):
        ...
        wm_frame = ttk.Frame(top); wm_frame.grid(row=0, column=4, rowspan=2, padx=8, sticky="w")
        ...
        left = ttk.Frame(mid, width=260); right = ttk.Frame(mid)
        mid.add(left, weight=1); mid.add(right, weight=3)

        ttk.Label(left, text="Posts").pack(anchor="w")
        self.lb_posts = tk.Listbox(left, height=20); self.lb_posts.pack(fill="both", expand=True, pady=4)
        self.lb_posts.bind("<<ListboxSelect>>", self.on_select_post)
        self.lbl_post_info = ttk.Label(left, text="0 posts"); self.lbl_post_info.pack(anchor="w")

        prev_top = ttk.Frame(right); prev_top.pack(fill="x")
        ...
        prev = ttk.Frame(right); prev.pack(fill="both", expand=True, pady=8)
        self.lbl_before = ttk.Label(prev, text="Before"); self.lbl_after = ttk.Label(prev, text="After")
        self.lbl_before.grid(row=0, column=0, sticky="nsew", padx=4)
        self.lbl_after.grid(row=0, column=1, sticky="nsew", padx=4)
        prev.columnconfigure(0, weight=1); prev.columnconfigure(1, weight=1); prev.rowconfigure(0, weight=1)

        bottom = ttk.Frame(self); bottom.pack(fill="x", padx=8, pady=6)
        self.progress = ttk.Progressbar(bottom, mode="determinate"); self.progress.pack(fill="x", expand=True, side="left", padx=4)
    ...
    def preview_clear(self):
        ...
        self.preview_tk_before = None; self.preview_tk_after = None
    ...
    def collect_settings(self) -> AppSettings:
        ...
        return AppSettings(
            input_root=in_root, output_root=out_root, sizes=sizes, bg_color=bg,
            wm_text=self.var_wm_text.get(), wm_opacity=int(self.var_wm_opacity.get()),
    ...
    def on_preview(self):
        ...
        if not sel:
            messagebox.showinfo("Preview", "Select a post from the left list."); return
        post_name = self.lb_posts.get(sel[0])
        if not self.posts.get(post_name):
            messagebox.showinfo("Preview", "No images in this post."); return

        settings = self.collect_settings()
        try:
            before_img, after_img = self.controller.preview_first_of_post(post_name, self.posts, settings)
        except Exception as e:
            messagebox.showerror("Preview Error", str(e)); return

        self.preview_tk_before = self.to_tk_image(before_img, 520, 520)
        self.preview_tk_after = self.to_tk_image(after_img, 520, 520)
    ...
    def on_start_batch(self):
        ...
        # progress/done callbacks marshal updates onto UI thread
        def on_progress(value):
            self.progress.after(0, self.progress.configure, {"value": value})
        def on_done(processed):
            self.progress.after(0, lambda: messagebox.showinfo("Done", f"Finished. Processed {processed} items."))
        def on_error(msg):
            self.progress.after(0, lambda: messagebox.showerror("Run Error", msg))
